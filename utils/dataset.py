...
class LocDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        left_bino_depth_path = pair.get('left_bino_depth_path', None)
        ...
        left_bino_depth_map = (
            self._load_depth(left_bino_depth_path, out_h=out_h, out_w=out_w)
            if left_bino_depth_path
            else left_mono_depth_map.clone()
        )
        ...
        if is_vigor:
            ...
            if roll_shift:
                ...
                if left_bino_depth_map is not None:
                    left_bino_depth_map = torch.roll(left_bino_depth_map, shifts=roll_shift, dims=-1)

        pair["left_image"] = left_image
        pair["sat_image"] = sat_image
        pair["left_mono_depth_map"] = left_mono_depth_map
        if left_bino_depth_map is not None:
            pair["left_bino_depth_map"] = left_bino_depth_map
    ...
    def collate_fn(self, pairs):
        ...
        if self.aug:
            ...
            if "left_bino_depth_map" in collated_pairs:
                collated_pairs["left_bino_depth_map"] = F.interpolate(
                    collated_pairs["left_bino_depth_map"].unsqueeze(1),
                    size=(aug_h, aug_w),
                    mode="bilinear",
                    align_corners=False,
                ).squeeze(1)
        ...
        if "left_bino_depth_map" in collated_pairs:
            label_data['left_bino_depth_map'] = collated_pairs["left_bino_depth_map"]
