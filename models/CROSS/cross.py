...
class CROSS(nn.Module):
    ...
    def _depth_mape_in_range(self, pred_depth_map, bino_depth_map):
        pred_depth_map = torch.nan_to_num(
            pred_depth_map,
            nan=self.structure_head.min_depth,
            posinf=self.structure_head.max_depth,
            neginf=self.structure_head.min_depth,
        )
        valid_mask = (
            torch.isfinite(pred_depth_map)
            & torch.isfinite(bino_depth_map)
            & (bino_depth_map >= self.structure_head.min_depth)
            & (bino_depth_map <= self.structure_head.max_depth)
        )
        if not torch.any(valid_mask):
            return 0.0

        ape = torch.abs(pred_depth_map - bino_depth_map) / bino_depth_map.clamp_min(1e-6)
        return torch.nan_to_num(ape[valid_mask], nan=0.0, posinf=0.0, neginf=0.0).mean().item()
    ...
    def loss(self, pred, label):
        ...
        depth_mape = (
            self._depth_mape_in_range(
                pred['left_depth_maps']['scale_1'],
                label['left_bino_depth_map'].to(self.device),
            )
            if 'left_bino_depth_map' in label
            else float('nan')
        )
